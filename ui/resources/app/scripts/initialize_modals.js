...
$('#donglein_key_setting_next').on('click', () => {
  ...
  setTimeout(() => {
    ...
    let random_string = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';

    for(let i = 0; i < 16; i++) {
      donglein_key += random_string.charAt(Math.floor(Math.random() * random_string.length));
    }

    let result = addon.set_donglein_key(donglein_key);
